...
namespace Net {
	...
	private:
		...
		Utils::QueueLF<Net::Message<T>> msgQueueOut;
		Utils::QueueLF<std::shared_ptr<Net::OwnerMessage<T>>>& msgQueueIn;

		Net::Message<T> temporaryMessage;

		OwnerConnection owner;
	public:
		Connection(OwnerConnection _owner, asio::io_service& _context, SOCKET _socket, Utils::QueueLF<std::shared_ptr<Net::OwnerMessage<T>>>& _msgIn)
	...
	private:
		void ReadHeader() {
			asio::async_read(connectSocket, boost::asio::buffer(&temporaryMessage.header, sizeof(Net::HeaderMessage<T>)),
		...
		void WriteHeader() {
			asio::async_write(connectSocket, boost::asio::buffer(&msgQueueOut.front().header, sizeof(Net::HeaderMessage<T>)),
		...
		void AddMessageToQueue() {
			if (owner == OwnerConnection::Server) {
				msgQueueIn.push_back(std::make_shared<Net::OwnerMessage<T>>(this->shared_from_this(), temporaryMessage));
			}
			else {
				msgQueueIn.push_back(std::make_shared<Net::OwnerMessage<T>>(nullptr, temporaryMessage));
			}
